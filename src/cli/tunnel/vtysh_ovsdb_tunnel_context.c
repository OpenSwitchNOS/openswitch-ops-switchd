                                  ...
                                  struct ovsdb_idl *idl)
{
    ...
    // Tunnel mode
    tunnel_id = get_id_from_name((const char*)if_row->name, "tunnel");
    ...
    if (src_intf)
    {
        int loopback_id = get_id_from_name(src_intf, "loopback");
...
void
print_tunnel_intf_running_config(const struct ovsrec_interface *if_row,
                                 struct ovsdb_idl *idl,
...
vtysh_tunnel_intf_context_clientcallback(void *p_private)
{
    ...
    {
        print_tunnel_intf_running_config(ifrow, p_msg->idl, p_msg,
                                         NULL /* vty */);
    }
