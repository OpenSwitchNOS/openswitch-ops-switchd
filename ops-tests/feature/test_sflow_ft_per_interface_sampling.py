...
def test_sflow_ft_per_interface_sampling(topology, step):
    ...
    assert str(sflow_config['agent_interface']) == '1'
    ...
    # Checking if FLOW packets generated from sflow disabled interface
    assert result['flow_count'] > 0
    for packet in result['packets']:
        if str(packet['packet_type']) == 'FLOW':
            assert str(packet['src_ip']) != '10.10.12.2'
            assert str(packet['src_ip']) != '10.10.12.1'
            assert str(packet['dst_ip']) != '10.10.12.2'
            assert str(packet['dst_ip']) != '10.10.12.1'
