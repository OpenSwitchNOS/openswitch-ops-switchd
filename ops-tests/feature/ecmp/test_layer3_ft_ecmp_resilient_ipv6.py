...
class GeneratorThread (threading.Thread):
    ...
    def run(self):
        ...
        while not exit_flag:
            generated_traffic = self.host('ping6 -c 10 -i 1 '
                                          '{0}'.format(self.dst))
...
def run_ecmp_cycle(hs1, nh1, nh2, nh3):
    ...
    nh1_thread = ReceiverThread('1001::1', nh1, '4001::1', '7000::1')
    nh2_thread = ReceiverThread('2001::1', nh2, '4001::1', '7000::1')
    nh3_thread = ReceiverThread('3001::1', nh3, '4001::1', '7000::1')
    hs1_thread = GeneratorThread('4001::1', hs1, '4001::1', '7000::1')
    ...
    hs1.libs.ping.ping(1, '1001::1')
    hs1.libs.ping.ping(1, '2001::1')
    hs1.libs.ping.ping(1, '3001::1')
...
def test_ecmp_ipv6(topology):
    ...
    with sw1.libs.vtysh.Configure() as ctx:
        ctx.ipv6_route('7000::/64', '1001::1')
        ctx.ipv6_route('7000::/64', '2001::1')
    ...
    nh1.libs.ip.add_route('4001::/64', '1001::2')
    nh2.libs.ip.add_route('4001::/64', '2001::2')
    nh3.libs.ip.add_route('4001::/64', '3001::2')
    ...
    # disturbed
    if nexthop_list[0] == '1001::1':
        with sw1.libs.vtysh.ConfigInterface('2') as ctx:
            ctx.shutdown()
    if nexthop_list[0] == '2001::1':
        with sw1.libs.vtysh.ConfigInterface('3') as ctx:
            ctx.shutdown()
    if nexthop_list[0] == '3001::1':
        with sw1.libs.vtysh.ConfigInterface('1') as ctx:
            ctx.shutdown()
    ...
    # disturbed
    if first_nexthop == '1001::1':
        with sw1.libs.vtysh.ConfigInterface('2') as ctx:
            ctx.no_shutdown()
    if first_nexthop == '2001::1':
        with sw1.libs.vtysh.ConfigInterface('3') as ctx:
            ctx.no_shutdown()
    if first_nexthop == '3001::1':
        with sw1.libs.vtysh.ConfigInterface('1') as ctx:
            ctx.no_shutdown()
