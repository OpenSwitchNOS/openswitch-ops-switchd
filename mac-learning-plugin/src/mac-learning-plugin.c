...
/* Updated MAC entries' list to be send down to PD, only if PD has registered API to get notification.
 *      As we process each L2 MAC table update from OVSDB, we will add MAC entry details to
 *          this list along with action (create/update/delete) taken.
 ...
 * Notes:    For debug purposes, this function can be used as PD's registered API
 */
/* static int
test_update_l2_mac_table(const struct ofproto *ofproto, struct ovs_list *mac_entry_list)
{
    ...
    VLOG_DBG("%s Processing Updated MAC list with %d MAC entries.", __FUNCTION__, (uint32_t)list_size(mac_entry_list));

    // Process Updated MAC list
    LIST_FOR_EACH (entry, node, mac_entry_list) {
        VLOG_DBG("%s: Entry from list - mac:" ETH_ADDR_FMT " vlan:%d port:%s action:%d\n",
                 __FUNCTION__, ETH_ADDR_ARGS(entry->mac), entry->vlan, entry->port_name, entry->action);
    }
    return 0;
}
*/
...
mac_hash_table_lookup_by_row(const struct ovsrec_mac *ovsdb_row)
{
    ...
    if (! ovs_scan(ovsdb_row->mac_addr, ETH_ADDR_SCAN_FMT, ETH_ADDR_SCAN_ARGS(mac))) {
        return NULL;
    }
    vlan = ovsdb_row->vlan;
...
mac_hash_table_entry_dump(struct ds *ds, struct mac_hash_table_entry *entry)
{
    ...
    ds_put_format(ds, "mac:" ETH_ADDR_FMT " vlan:%-4d from:%-10s dest:%s\n",
                  ETH_ADDR_ARGS(entry->mac), entry->vlan, entry->from, entry->dest);
}
 ...
 * Purpose:  add hash entry's details to Updated MAC entries's list, to be send down to PD later
 * In:       entry - pointer to hash table entry
 ...
 */
static void
mac_entry_add_to_updated_list(const struct mac_hash_table_entry *entry, mac_table_action action)
{
    ...
    VLOG_DBG("%s: Added MAC entry to updated MAC list - mac:" ETH_ADDR_FMT " vlan:%d port:%s action:%d",
             __FUNCTION__, ETH_ADDR_ARGS(list_entry->mac), list_entry->vlan, list_entry->port_name, action);
}
 ...
 *              add this entry in Updated MAC entries' list to be send down to PD, if needed
 * In:       ovsdb_row - pointer to OVSDB MAC table row
...
mac_entry_add(const struct ovsrec_mac *ovsdb_row, unsigned int seqno)
{
    ...
    if (! ovs_scan(ovsdb_row->mac_addr, ETH_ADDR_SCAN_FMT, ETH_ADDR_SCAN_ARGS(hash_entry->mac))) {
       VLOG_ERR("%s: OVSDB MAC table row has incorrect MAC address", __FUNCTION__);
       free(hash_entry);
       return NULL;
    }
    ...
    VLOG_DBG("%s: Added MAC entry to MAC hash table - mac:" ETH_ADDR_FMT " vlan:%d from:%s dest:%s\n",
 ...
 * Purpose:  function to check any possible conflicts by updating current mac hash entry with
 *              new information from OVSDB
 * In:       ovsdb_row - pointer to OVSDB MAC table row
 * In:       entry - pointer to hash table entry
 * Return:   true if allowing this change from OVSDB would cause conflicts, false otherwise
 * Notes:    Need for conflict checks.
 *              OVSDB MAC table schema's index key:{bridge,mac,vlan,from}
 *              PI MAC table's index key: {mac,vlan}
 *              Since we only support single instance of bridge, not having bridge in PI table's key is OK.
 *              Ideally "from" should be removed from OVSDB MAC table schema's index and have public API available
 *                  in ovs_util repo, that can be used by any other repo, to check for conflicts before making changes to
 *                  OVSDB MAC table. After that, PI MAC table could simply be a reflection of the OVSDB table.
 *              Until we have that in place, each update from OVSDB needs to be checked for possible conflicts.
 *
 *           Do not allow MAC entry, learned from one source, and be updated to another source later.
 *              For example, entry added by local learn (dynamic) can NOT be overwritten by remote learn (hw-vtep).
 *              We are implementing FIFO, so who ever gets in first will remain in table until it ages out or been removed.
 */
static bool
mac_entry_check_for_conflicts(const struct ovsrec_mac *ovsdb_row, struct mac_hash_table_entry *entry)
{
    if ((! ovsdb_row) || (! entry)) {
        return true;
    }

    if (strcmp(entry->from, ovsdb_row->from) == 0) {
        return false;
    }
 ...
 * In:       entry - pointer to hash table entry
 * Return:   none
 * Notes:    Updating this entry should NOT cause as conflicts, as all possible conflicts should
 *              have been checked for already before this function call.
 *           Note about fields that can be updated or not.
 *              {mac+vlan} fields are used for hash-key calculation.
 *              'from' field, source from where we learned about this host is not allowed to be changed.
 *              Always update 'idl_seqno' field.
 *              Field that can also be updated is 'dest' meaning a host has moved from one port to the other.
 */
static void
mac_entry_update(const struct ovsrec_mac *ovsdb_row, unsigned int seqno, struct mac_hash_table_entry *entry)
{
    ...
    VLOG_DBG("%s: Updated MAC entry to MAC hash table - mac:" ETH_ADDR_FMT " vlan:%d from:%s dest:%s\n",
             __FUNCTION__, ETH_ADDR_ARGS(entry->mac), entry->vlan, entry->from, entry->dest);
 ...
 */
static void
mac_entry_delete(const struct ovsrec_mac *ovsdb_row, unsigned int seqno, struct mac_hash_table_entry *entry)
{
    if (! entry) {
        return;
    }

    /* Add this entry to Updated MAC entries' list to be send down to PD, if needed */
    if (notify_pd_for_changes) {
    ...
    VLOG_DBG("%s: Deleted MAC entry from MAC hash table - mac:" ETH_ADDR_FMT " vlan:%d from:%s dest:%s\n",
             __FUNCTION__, ETH_ADDR_ARGS(entry->mac), entry->vlan, entry->from, entry->dest);
 ...
 * In:       blk_params - need at least following elements: br, ofproto, idl, idl_seqno
 * Return:   none
 */
static void
mac_learning_callback_bridge_reconfig (struct blk_params *blk_params)
{
    ...
    if ((! blk_params) || (! blk_params->br) || (! blk_params->ofproto) || (!blk_params->idl)) {
        VLOG_DBG("%s:feature plugin callback called without setting up parameters", __FUNCTION__);
        return;
    }
    ...
    VLOG_DBG("%s: Beginning to process L2 MAC table updates from OVSDB for idl-seq=%d, bridge=%s",
             __FUNCTION__, idl_seqno, blk_params->br->name);

    if (mac_entry_row) {
        mac_entries_created = OVSREC_IDL_ANY_TABLE_ROWS_INSERTED(mac_entry_row, idl_seqno);
        mac_entries_modified = OVSREC_IDL_ANY_TABLE_ROWS_MODIFIED(mac_entry_row, idl_seqno);
    ...
    if (mac_entries_created || mac_entries_modified || mac_entries_deleted) {
        ...
        OVSREC_MAC_FOR_EACH_SAFE(mac_entry_row, mac_entry_row_next, idl) {
            ...
            if (strcmp(mac_entry_row->bridge->name, blk_params->br->name) != 0) {
                continue;
            }
            ...
            if (! entry) {
                ...
                if (row_changed) {
                    ...
                    is_conflict = mac_entry_check_for_conflicts(mac_entry_row, entry);
                    ...
                    if (! is_conflict) {
                        ...
                        /* TODO: We can possibly delete OVSDB row since it's causing a conflict. */
                        VLOG_WARN("%s: MAC entry (mac:%s vlan:%d from:%s) from OVSDB rejected because of conflict.",
                                  __FUNCTION__, mac_entry_row->mac_addr, (int)mac_entry_row->vlan, mac_entry_row->from);

                    }

                } else if (strcmp(entry->from, mac_entry_row->from) == 0) {

                    /* Update idl_seqno for matching entry. idl_seqno is use as mark/sweep to delete unused MAC entries.
                     * We always update these, even if nothing has changed, because ovsdb_row may have changed out from under us.
                     */
        ...
        VLOG_DBG("%s: No changes in L2 MAC table from OVSDB for idl-seq=%d, bridge=%s",
                 __FUNCTION__, idl_seqno, blk_params->br->name);
        return;
    }
    ...
    if (mac_entries_deleted) {
        ...
        HMAP_FOR_EACH_SAFE (entry, next_entry, mac_hash_table_node, &mac_hash_table) {
    ...
    if (notify_pd_for_changes && entries_changed) {
        ...
        ret = p_asic_plugin_interface->update_l2_mac_table(blk_params->ofproto, &mac_list);

        if (ret) {
            VLOG_WARN("%s: L2 MAC table update from PD failed with %d changed MAC entries, idl-seq=%d",
...
mac_hash_table_init(void)
{
    ...
    unixctl_command_register("mac/show", "", 0, 0, mac_hash_table_unixctl_show, NULL);

    /* Check to see if PDL has registered API to get notification about updates in L2 MAC table */
    if (p_asic_plugin_interface && p_asic_plugin_interface->update_l2_mac_table) {
        notify_pd_for_changes = true;
        list_init(&mac_list);
    }

    VLOG_INFO("%s: asic_plugin=%p notify_pd=%d", __FUNCTION__, p_asic_plugin_interface, notify_pd_for_changes);
}
...
void init (int phase_id)
{
    ...
    /* Initialize MAC learning ASIC plugin -- it will find its ASIC provider APIs. */
    ...
    VLOG_INFO("[%s] Registering switchd plugin - phase_id: %d", MAC_LEARNING_PLUGIN_INTERFACE_NAME, phase_id);
    register_plugin_extension(&mac_learning_extension);

    VLOG_INFO("[%s] Registering BLK_BRIDGE_INIT", MAC_LEARNING_PLUGIN_INTERFACE_NAME);
    ...
    VLOG_INFO("[%s] Registering BLK_BR_FEATURE_RECONFIG", MAC_LEARNING_PLUGIN_INTERFACE_NAME);
