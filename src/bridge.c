...
port_configure(struct port *port)
{
    ...
    s.vlan = -1;
#ifdef OPS
    if (cfg->vlan_tag && ops_port_get_tag(cfg, idl) >= 1 && ops_port_get_tag(cfg, idl) <= 4094) {
#else
    if (cfg->vlan_tag && ops_port_get_tag(cfg, idl) >= 0 && ops_port_get_tag(cfg, idl) <= 4095) {
#endif
        s.vlan = ops_port_get_tag(cfg, idl);
    }
    ...
    if (cfg->n_vlan_trunks) {
        ...
        for (index = 0; index < cfg->n_vlan_trunks; index++) {
            vlan_trunks[index] = ops_port_get_trunks(cfg, index, idl);
        }
    ...
    HMAP_FOR_EACH (port, hmap_node, &br->ports) {
        ...
        if (port->cfg->mac && eth_addr_from_string(port->cfg->mac, &iface_ea)) {
            ...
            if (fake_br && fake_br->cfg && fake_br->cfg->vlan_tag) {
                ...
                if (ops_port_get_tag(port->cfg, idl) != ops_port_get_tag(fake_br->cfg, idl)) {
                    continue;
                }
            }
...
collect_splinter_vlans(const struct ovsrec_open_vswitch *ovs_cfg)
{
    ...
    for (i = 0; i < ovs_cfg->n_bridges; i++) {
        ...
        for (j = 0; j < br_cfg->n_ports; j++) {
            ...
            if (port_cfg->vlan_tag && ops_port_get_tag(port_cfg, idl) > 0 && ops_port_get_tag(port_cfg, idl) < 4095) {
                bitmap_set1(splinter_vlans, ops_port_get_tag(port_cfg, idl));
            }
        }
    }
