...
/* TODO: mph: note, this was in lib/openswitch-idl.h pre-split. new home?? */
/****************************** MIRROR TABLE *******************************/
...
bridge_configure_mirrors(struct bridge *br)
{
    ...
    HMAP_FOR_EACH_SAFE (m, next, hmap_node, &br->mirrors) {
        ...
        if (ovsdb_datum_find_key(mc, &atom, OVSDB_TYPE_UUID) == UINT_MAX) {
        ...
        } else if (m->cfg->active && (*m->cfg->active == false)) {
            ...
            smap_replace(&smap, MIRROR_CONFIG_OPERATION_STATE, "shutdown");
        ...
        if (err != 0) {
            smap_replace(&smap, MIRROR_CONFIG_OPERATION_STATE, "destroy_failed");
            VLOG_WARN("Failed to destroy deleted mirror %s", m->cfg->name);
        }
    ...
    for (i = 0; i < br->cfg->n_mirrors; i++) {
        ...
        if (!m) {
             ...
             */
            if (!OVSREC_IDL_IS_ROW_MODIFIED(cfg, idl_seqno)) {
                continue;
            }
        ...
        if (err == 0) {
            ...
            smap_replace(&smap, MIRROR_CONFIG_OPERATION_STATE, "active");

        } else {

            smap_replace(&smap, MIRROR_CONFIG_OPERATION_STATE, "configure_failed");
            VLOG_WARN("Failed to (re)configure mirror %s", m->cfg->name);
            /* configure failed, attempt to destroy */
            err = mirror_destroy(m);

            if (err == 0) {
                smap_replace(&smap, MIRROR_CONFIG_OPERATION_STATE, "destroy_failed");
                VLOG_WARN("Failed to destroy failed-configure mirror %s", m->cfg->name);
            }
