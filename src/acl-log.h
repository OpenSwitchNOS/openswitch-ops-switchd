/* Copyright (C) 2016 Hewlett Packard Development Company LP
 *
...
typedef struct {
   ...
   uint32_t    list_type;    /**< The ACL type, e.g., IPv4. The values are the
                               same as for ops_cls_type. */
   char        list_name[32 + 1]; /**< The name of the ACL. */
...
/**
 * The sequence number used for signaling that an ACL logging packet has been
 * received and is ready for processing.
 *
 * @return The ACL logging sequence number.
 *
 */
 ...
 *
 * @return The available information about a received packet.
 *
 */
acl_log_info_t acl_log_pkt_data_get(void);

/**
 * This function accepts information about a packet received for ACL
 * logging.
 *
 * @param The available information about a received packet.
 *
 */
void acl_log_pkt_data_set(acl_log_info_t new_pkt);
