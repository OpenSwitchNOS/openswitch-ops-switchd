...
def test_subinterface_route(topology):
    ...
    while pass_flag == 0 and attemps > 0:
        ...
        switch2_routes = sw1.libs.vtysh.show_ip_route()
        sw1_route = sw1("ip netns exec swns route", shell='bash')
        sw2_route = sw2("ip netns exec swns route", shell='bash')
        lines = sw1_route.split('\n')
        sw1_count = len(lines) - header_line_count
        lines = sw2_route.split('\n')
        sw2_count = len(lines) - header_line_count
        if len(switch1_routes) == 3 and\
           len(switch2_routes) == 3 and\
           sw1_count == 3 and sw2_count == 3:
            pass_flag = 1
        else:
            pass_flag = 0
            attemps -= 1
