...
void mac_learning_init (void);
void mac_learning_wait(void);
void mac_learning_reconfigure_init(void);
...
static struct mac_learning_plugin_interface mac_learning_plugin = {
    .mac_learning_init = &mac_learning_init,
    .mac_learning_wait = &mac_learning_wait,
    .mac_learning_reconfigure = &mac_learning_reconfigure_init,
};
...
void mac_learning_reconfigure_init(void) //struct blk_params *blk_p)
{
    uint64_t seq = seq_read(mac_learning_trigger_seq_get());
    if (seq != mlearn_seqno) {
        mlearn_seqno = seq;
        mac_learning_check_seq();//blk_p);
    }
}
                                  ...
                                  struct ovsdb_idl_txn *mac_txn)
{
    ...
    VLOG_DBG("%s: adding mac: %s, vlan: %d, bridge: %s, port: %s, from: learning",
 ...
 */
static void
mlearn_plugin_db_del_local_mac_entry (struct ofproto_mlearn_hmap_node *mlearn_node)
{
    const struct ovsrec_mac *mac_e = NULL;
    struct bridge *br = NULL;
    struct port *port = NULL;
    ...
    if (port) {
        ...
        VLOG_DBG("%s: deleting mac: %s, vlan: %d, bridge: %s, port: %s, from: learning",
              __FUNCTION__, str, mlearn_node->vlan, br->name, port->name);
...
/*
 * Function: mac_learning_check_seq
 *
 ...
 * ofproto functions to get the hmap populated during MAC learning.
 *
 * It creates, removes entries in MAC table depending on the operation.
 */
static void
mac_learning_check_seq(void)
{
