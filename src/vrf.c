                            ...
                            struct ofproto_route *ofp_route)
{
    ...
    for (i = 0; i < route_row->n_nexthops; i++) {
        ...
        if (vrf_is_nh_row_selected(nh_row) && (nh_row->ip_address ||
           ((nh_row->n_ports > 0) && nh_row->ports[0]))) {
            /* TODO: Shall we add to hash, if we are not passing to PD */
        ...
        if (vrf_is_nh_row_selected(nh_row) && (nh_row->ip_address ||
           ((nh_row->n_ports > 0) && nh_row->ports[0]))) {
            ...
            if (nh_entry->port_name) { /* nexthop is a port */
                ...
                VLOG_DBG("%s : nexthop port : (%s)", __func__,
                         nh_entry->port_name);
                ofp_route->n_nexthops++;
            } else {
    ...
    /* Return the resolved NH list */
    if (ofp_route->n_nexthops > 0) {
        VLOG_DBG("Returning with %d resolved NH", ofp_route->n_nexthops);
        return ofp_route->n_nexthops;
    } else {
        /* If none is resolved, fill atleast one for ASIC */
        VLOG_DBG("Filling atleast one un-resolved NH for asic");
        ofp_route->n_nexthops = 0;
        /* Fill all port based nexthops first */
        for (i = 0; i < route_row->n_nexthops; i++) {
            nh_row = route_row->nexthops[i];

            if (vrf_is_nh_row_selected(nh_row) &&
               ((nh_row->n_ports > 0) && nh_row->ports[0])) {
                nh_entry = vrf_route_nexthop_lookup(route_entry, NULL,
                          nh_row->ports[i]->name);
                if (nh_entry == NULL) {
                    VLOG_ERR("NH not in route->nh hash");
                    continue;
         ...
         /* If non port based NH, fill one ip based to Copy2Cpu */
         if (ofp_route->n_nexthops == 0) {
            for (i = 0; i < route_row->n_nexthops; i++) {
                 nh_row = route_row->nexthops[i];
                 if (vrf_is_nh_row_selected(nh_row) && (nh_row->ip_address)) {
                ...
                if (ofp_route->n_nexthops > 0) {
                    VLOG_DBG("Filled one un-resolved NH");
                    return ofp_route->n_nexthops;
                }
            }
        }
...
static void
vrf_ofproto_set_resolved_nh(struct vrf *vrf, struct ofproto_route *ofp_route,
                   struct nexthop *nh, struct ofproto_route_nexthop *ofp_nh)
{
    struct neighbor *neighbor;
    ...
    SHASH_FOR_EACH(shash_idl_nh, &current_idl_nhs) {
        ...
        if (!nh) {
            ...
            **  Neighbor table - This will be ecmp case of more than 1 NH */
            if ((nh = vrf_nexthop_add(vrf, route, nh_row))) {
                if (route_row->n_nexthops > 1) {
                    vrf_ofproto_set_resolved_nh(vrf, &ofp_route, nh,
                                &ofp_route.nexthops[ofp_route.n_nexthops]);
                } else {
                    vrf_ofproto_set_nh(vrf,
                                    &ofp_route.nexthops[ofp_route.n_nexthops],
