...
# Bridging scenario through Open Flow 1.3 using ovs-ofctl utility on openswitch
# ####################### END ###############################
...
ops1:port1
"""
# Global variables for tcp port, flow / group configuration in a dictionary for multi-purpose use
# "config" dictionary will be parsed to create a ops-ofctl command for add / get flows / groups
# We have defined predefined attribute list for flow / groups for verifying the flow / group stats
tcp_port = 6643
config = {
              "flow": [{
                "table": 10,
...
# Test case method contains pre-defined variables with default values and initialized the node identifier
# Using this method we are parsing the "config" dictionary to prepare ovs-ofctl add flow / group commands
# We are creating a user defined bridge as "bridge_ovs_openflow" and adding ports to it.
...
# topology fixture allows us to create python switch objects to represent topology devices
def test_ovs_openflow(topology):
    ...
    group_ids = list()
    try:
        # Fetching the ip address of the eth0 interface by passing open switch object
        # ip_address = get_eth_ip_address(ops1) # Don't need to fetch the service IP address
        # print(ip_address)
        # Creating user defined bridge and enabling the open flow agent in passive mode
        ...
        # ops1("ovs-vsctl " # Earlier we have set the controller in passive mode, now commented as we don't need it
        # "set-controller "+bridge_name+" "
        # "ptcp:"+str(tcp_port), shell="bash")
        if len(ports_list) > 0:
            ...
            for port in ports.split("\n"):
                port = unicode_to_string(port) if isinstance(port, unicode) else port
        ...
        sleep(5)
        # process = ops1("netstat -at | grep "+str(tcp_port), shell="bash")
        # restart_open_switchd(ops1, process, 0) # No need to check for the set-controller process as we don't need it
        bridge_details = ops1(ovs_ofctl_cmd+" show "+bridge_name, shell="bash")
        # set_trace()
        re.search('\saddr:(.*)\s', bridge_details).group(1)
        # Iterating the "config" dictionary w.r.t flow and groups to prepare the ovs-ofctl
        # commands. Installing flows /groups with the commands and verifying them by fetching them back
        # Once the verification is successful, we are deleting the flow / group and verifying the same.
        for label, data in config.iteritems():
                ...
                if label == "flow":
                    for flow in data:
                        ...
                        if isinstance(flow, dict):
                            ...
                            for field, value in flow.iteritems():
                                if field != "actions":
                                    ...
                                    if not isinstance(value, basestring):
                                        for sub_field in value:
                                            for act_field, act_val in sub_field.iteritems():
                                                if act_field != "set_field":
                                                    ...
                                                    if isinstance(act_val, dict):
                                                        for action, sub_value in act_val.iteritems():
                            ...
                            for field, value in group.iteritems():
                                if field != "bucket":
                                    ...
                                    for bucket_data in value:
                                        for bkt_field, bkt_value in bucket_data.iteritems():
...
# Method to check the open-switchd process and re-starting for one time if it is not started
...
# Method to parse add verify the configuration based on the provided arguments with the "config" dictionary
            ...
            for sub_list in output:
                    if type == "flow":
                        if sub_list[0] in flow_attrs or re.search("goto_table:\d", sub_list[0]):
                            ...
                            if not isinstance(flow_config, basestring):
                                    ...
                                    if str(flow_data.get("table")) == str(kwargs.get("table")):
                                        if isinstance(flow_data, dict):
                                            ...
                                            if sub_list[0] != "actions":
                                                ...
                                                if not isinstance(action_list, basestring):
                                                    for action in action_list:
                                                        if isinstance(action, dict):
                                                            for field, value in action.iteritems():
                                                                if not re.search("set_field:(.*)", sub_list[1]) and field != "set_field":
                                                                    ...
                                                                    if isinstance(value, dict) and re.search("set_field:(.*)", sub_list[1]):
                                                                        for set_fields, field_values in value.iteritems():
                        ...
                        if sub_list[0] in group_attrs:
                            ...
                            if not isinstance(group_config, basestring):
                                                ...
                                                if not isinstance(group_config, basestring):
                                                    for bucket in bucket_data:
                                                        if isinstance(bucket, dict):
                                                            for field, value in bucket.iteritems():
...
# Common method to print the exception message along with the line number for easy understanding
...
# Conversion method used to convert the unicode string to a ascii string
