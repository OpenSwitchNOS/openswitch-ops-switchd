 ...
 * This should be used to specify the API function pointers.
 *
 * Currently there are no functions which needs to be exposed so that
 * other files can use. In future when the APIs need to be exposed
 * add it here and in the mac-learning-plugin.h as well.
 */
