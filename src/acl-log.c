...
static acl_log_info_t info OVS_GUARDED_BY(acl_log_mutex) = { .valid_fields = 0 };
...
void
acl_log_pkt_data_set(acl_log_info_t new_pkt)
{
   /* take the mutex */
   ovs_mutex_lock(&acl_log_mutex);

   /* copy the argument into the static value */
   memcpy(&info, &new_pkt, sizeof(acl_log_info_t));
