 ...
 *
 *   We only support a single bridge instance for now, hence this structure doesn't care
 *      which bridge a MAC entry is associated with. When multiple bridges are supported
 *      a new element 'bridge' needs to be added and have it part of hash-key calculation.
 *
...
struct mac_hash_table_entry {
    ...
    char               from[10];                    /* source from where this entry is learned (dynamic, hw-vtep etc)  */
    unsigned int       idl_seqno;                   /* last seen idl seq# for mark/sweep operation to identify deleted rows */
};
...
/* Notes:
 *   With every change in L2 MAC table, coming from OVSDB, PI notifies PD about them,
 *      provided PD has registered update_l2_mac_table() via ASIC plug-in registration.
 */
