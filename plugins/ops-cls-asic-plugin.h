...
{
    uint32_t            entry_flags;      /**< bitwise-OR of flags defined in
                                               @ref ops_cls_list_entry_flags */
    union
    {
        struct in6_addr v6;
        struct in_addr  v4;
    ...
    uint16_t            L4_src_port_min;    /**< Minimum TCP/UDP source port,
                                                 used as minimum parameter when
                                                 op is range, and only
                                                 parameter when op is lt, gt,
                                                 eq and neq */
    uint16_t            L4_src_port_max;    /**< Maximum TCP/UDP source port,
                                                 used as maximum value
                                                 parameter when op is range */
    uint16_t            L4_dst_port_min;    /**< Minimum TCP/UDP destination
                                                 port used as minimum parameter
                                                 when op is range, and only
                                                 parameter when op is lt, gt,
                                                 eq and neq */
    uint16_t            L4_dst_port_max;    /**< Maximum TCP/UDP destination
                                                 port used as maximum value
                                                 parameter when op is range */
    ...
    uint8_t             protocol;           /**< IP protocol number,
                                                 e.g. tcp=6, udp=17 */
    uint8_t             tos;                /**< IP Type of Service/DiffServ
                                                 Code Point (DSCP) */
    ...
    uint8_t             L2_cos;             /**< 802.1p Class of Service (CoS)/
...
{
    uint32_t         action_flags; /**< bitwise-OR of flags defined in
                                        @ref ops_cls_list_entry_action_flags */
...
{
    struct ops_cls_list_entry_match_fields entry_fields; /**< field(s)/value(s)
                                                              to match */
...
struct ops_cls_list
{
    struct uuid                 list_id;        /**< uuid of classifier list
                                                     in OVSDB */
    const char *                list_name;      /**< name of classifier list */
    enum ops_cls_type           list_type;      /**< type of classifier list
                                                     aclv4, aclv6, aclMac */
    struct ops_cls_list_entry   *entries;       /**< array of classifier list
                                                     entries */
    uint16_t                    num_entries;    /**< number of entries in
                                                     classifier list */
};
...
{
    enum ops_cls_list_pd_status_code    status_code; /**< Status of this port's
                                                          set operation */
...
{
    enum ops_cls_list_pd_status_code    status_code; /**< Status of this list
                                                          define operation */
    uint16_t                            entry_id;    /**< First entry that
                                                          encountered an error,
                                                          0 based index into
