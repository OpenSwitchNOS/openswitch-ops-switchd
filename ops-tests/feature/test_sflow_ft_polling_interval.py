...
# Nodes
[type=openswitch name="OpenSwitch 1" image="openswitch/sflow:latest"] ops1
...
def test_sflow_ft_polling_interval(topology, step):
    ...
    for packet in result['packets']:
        if str(packet['packet_type']) == 'CNTR':
                ...
                if packet['if_index'] not in index:
                    # step("\n\nindex----"+packet['if_index'])
                    assert packet['agent_address'] == '10.10.10.1'
                    index[i] = packet['if_index']
                    i = i+1
    # step("\n\nindex count----"+str(i))
    ...
    # Configure new polling interval
    with ops1.libs.vtysh.Configure() as ctx:
