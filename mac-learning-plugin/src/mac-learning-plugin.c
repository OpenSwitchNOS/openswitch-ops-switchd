...
/*
 * Function: mlearn_plugin_db_move_local_mac_entry
 *
 * This function takes the hmap node and update the move values to the corresponding
 * entry of MAC table in OVSDB.
 */
static void
mlearn_plugin_db_move_local_mac_entry (
                                  struct mlearn_hmap_node *mlearn_node,
                                  struct ovsdb_idl_txn *mac_txn)
{
    const struct ovsrec_mac *mac_e = NULL;
    ...
    /* MAC Entry not found to update move state, consider as new entry */
    if (!found)   {
        mac_e = ovsrec_mac_insert(mac_txn);
        ovsrec_mac_set_bridge(mac_e, br->cfg);
        ovsrec_mac_set_from(mac_e, OVSREC_MAC_FROM_DYNAMIC);
        ...
        ovsrec_mac_set_port(mac_e, port->cfg);
        ovsrec_mac_set_vlan(mac_e, mlearn_node->vlan);
        VLOG_WARN("%s: %s: Move miss vlan: %d, bridge: %s, port: %s, from: %s",
          __FUNCTION__, str, mlearn_node->vlan, br->name, port->name,
          OVSREC_MAC_FROM_DYNAMIC);
    }
}
    ...
    if (mhmap) {
        ...
        HMAP_FOR_EACH(mlearn_node, hmap_node, &(mhmap->table)) {
            if (mlearn_node->oper == MLEARN_ADD) {
                /* add new learnt mac to MAC table*/
                mlearn_plugin_db_add_local_mac_entry(mlearn_node, mac_txn);
            } else if (mlearn_node->oper == MLEARN_MOV) {
                /* Update the mac move state to MAC table */
                mlearn_plugin_db_move_local_mac_entry(mlearn_node, mac_txn);
            } else {
